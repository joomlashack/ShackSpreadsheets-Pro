...
class PHPExcel_Writer_Excel2007_ContentTypes extends PHPExcel_Writer_Excel2007_WriterPart
{
    ...
    public function writeContentTypes(PHPExcel $pPHPExcel = null, $includeCharts = false)
    {
        ...
        // Theme
        $this->writeOverrideContentType($objWriter, '/xl/theme/theme1.xml',
            'application/vnd.openxmlformats-officedocument.theme+xml');

        // Styles
        $this->writeOverrideContentType($objWriter, '/xl/styles.xml',
        ...
        if ($pPHPExcel->hasMacros()) { //Macros in workbook ?
            ...
            $this->writeOverrideContentType($objWriter, '/xl/workbook.xml',
            ...
            if ($pPHPExcel->hasMacrosCertificate()) {// signed macros ?
                ...
                $this->writeOverrideContentType($objWriter, '/xl/vbaProjectSignature.bin',
        ...
        // DocProps
        $this->writeOverrideContentType($objWriter, '/docProps/app.xml',
            'application/vnd.openxmlformats-officedocument.extended-properties+xml');

        $this->writeOverrideContentType($objWriter, '/docProps/core.xml',
            'application/vnd.openxmlformats-package.core-properties+xml');

        $customPropertyList = $pPHPExcel->getProperties()->getCustomProperties();
        if (!empty($customPropertyList)) {
            $this->writeOverrideContentType($objWriter, '/docProps/custom.xml',
        ...
        for ($i = 0; $i < $sheetCount; ++$i) {
            $this->writeOverrideContentType($objWriter, '/xl/worksheets/sheet' . ($i + 1) . '.xml',
        ...
        for ($i = 0; $i < $sheetCount; ++$i) {
            ...
            if (($drawingCount > 0) || ($chartCount > 0)) {
                $this->writeOverrideContentType($objWriter, '/xl/drawings/drawing' . ($i + 1) . '.xml',
            ...
            if ($chartCount > 0) {
                for ($c = 0; $c < $chartCount; ++$c) {
                    $this->writeOverrideContentType($objWriter, '/xl/charts/chart' . $chart++ . '.xml',
        ...
        for ($i = 0; $i < $sheetCount; ++$i) {
            if (count($pPHPExcel->getSheet($i)->getComments()) > 0) {
                $this->writeOverrideContentType($objWriter, '/xl/comments' . ($i + 1) . '.xml',
            ...
            if (count($pPHPExcel->getSheet()->getHeaderFooter()->getImages()) > 0) {
                foreach ($pPHPExcel->getSheet()->getHeaderFooter()->getImages() as $image) {
                    if (!isset($aMediaContentTypes[strtolower($image->getExtension())])) {
                        ...
                        $this->writeDefaultContentType($objWriter, strtolower($image->getExtension()),
