    ...
    private function match($token)
    {
        switch ($token) {
            ...
            default:
                ...
                if (preg_match('/^\$?[A-Ia-i]?[A-Za-z]\$?[0-9]+$/', $token) and !preg_match("/[0-9]/",
                        $this->lookAhead) and ($this->lookAhead != ':') and ($this->lookAhead != '.') and ($this->lookAhead != '!')) {
                    return $token;
                } elseif (preg_match("/^" . self::REGEX_SHEET_TITLE_UNQUOTED . "(\:" . self::REGEX_SHEET_TITLE_UNQUOTED . ")?\!\\$?[A-Ia-i]?[A-Za-z]\\$?[0-9]+$/u",
                        $token) and !preg_match("/[0-9]/",
                        $this->lookAhead) and ($this->lookAhead != ':') and ($this->lookAhead != '.')) {
                    ...
                    return $token;
                        ...
                        $token) && !preg_match("/[0-9]/", $this->lookAhead)) {
                    ...
                    return $token;
                        ...
                        $token) and !preg_match("/[0-9]/", $this->lookAhead)) {
                    ...
                    return $token;
                        ...
                        $token) and $this->lookAhead != '"' and (substr_count($token, '"') % 2 == 0)) {
