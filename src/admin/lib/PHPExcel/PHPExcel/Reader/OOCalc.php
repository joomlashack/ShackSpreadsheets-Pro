...
//        echo '<h1>Workbook Content</h1>';
        $xml               = simplexml_load_string($this->securityScan($zip->getFromName("content.xml")),
            'SimpleXMLElement', PHPExcel_Settings::getLibXmlLoaderOptions());
        $namespacesContent = $xml->getNamespaces(true);
//        echo '<pre>';
...
//                                    echo 'Formula: ', $cellDataFormula, PHP_EOL;
                                    ...
                                    foreach ($temp as &$value) {
                                        ...
                                        if ($tKey = !$tKey) {
                                            ...
                                            $value = PHPExcel_Calculation::translateSeparator(';', ',', $value,
                                                $inBraces);
                                        }
                                    }
                                    unset($value);
                                        ...
                                        if ($type !== PHPExcel_Cell_DataType::TYPE_NULL) {
                                            for ($rowAdjust = 0; $rowAdjust < $rowRepeats; ++$rowAdjust) {
                                                ...
                                                $objPHPExcel->getActiveSheet()
                                                    ...
                                                    ->setValueExplicit((($hasCalculatedValue) ? $cellDataFormula : $dataValue),
                                                        $type);
                                                if ($hasCalculatedValue) {
